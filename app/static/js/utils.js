    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            var header =
                ...
                '<div class="card-body"> ' +
                '<textarea class="form-control" id="novaMensagemForum' + tarefa + '" style="margin-bottom: 1rem;" name="mensagem" value=""  required="" autofocus></textarea> ' +
                '<button class="btn btn-outline-secondary" onClick="appendForum(' + tarefa + ')">Publicar </button> ' +
                '</div> ' +
                '</div> ' +
                '</div> ' +
                '</div> ' +
...
function appendForum(tarefa) {

    var comentario = $('#novaMensagemForum' + tarefa).val();
